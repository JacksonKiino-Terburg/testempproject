...
COPY . /opt/testempproject

RUN \
  pip3 install -r /opt/testempproject/docs/requirements.txt \
    && \
...
RUN \
  chgrp --recursive user /opt/testempproject \
    && \
  chmod --recursive g+rwx /opt/testempproject \
    && \
  echo "user granted permissions to /opt/testempproject"

USER user

RUN \
  cd /opt/testempproject \
    && \
